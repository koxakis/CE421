...
__global__ void histogram_equalizationGPU ( unsigned char * d_img_out, unsigned char * d_img_in,
											int * d_hist_in, int img_size, int nbr_bin, int * d_lut, int cdf, int min, int d) {
...
int main(int argc, char *argv[]){
	...
	hist_buffer = (int*)malloc(256 * sizeof(int));
	...
	//histogramGPU<<<1, 256>>>(d_hist_out, d_img_in, h_img_in.h * h_img_in.w, 256);
	cudaCheckError();

	cudaDeviceSynchronize();
	cudaCheckError();

	//cudaMemcpy(hist_buffer, d_hist_out, 256 * sizeof(int), cudaMemcpyDeviceToHost);
	//cudaCheckError();

	for ( i = 0; i < 256; i ++){
        hist_buffer[i] = 0;
    }
	unsigned char * img_in_test = h_img_in.img;
	for ( i = 0; i < h_img_in.h * h_img_in.w; i ++){
        hist_buffer[img_in_test[i]] ++;
    }
	while(min == 0){
		min = hist_buffer[i++];
	}
	d = h_img_in.h * h_img_in.w - min;
	for(i = 0; i < 256; i ++){
		cdf += hist_buffer[i];
		lut_buff[i] = (int)(((float)cdf - min)*255/d + 0.5);
		if(lut_buff[i] < 0){
			lut_buff[i] = 0;
		}
	...
	cudaMemcpy(d_hist_out, hist_buffer, 256 * sizeof(int), cudaMemcpyHostToDevice);
	cudaCheckError();
	cudaMemcpy(d_lut, lut_buff, 256 * sizeof(int), cudaMemcpyHostToDevice);
	cudaCheckError();
	...
	printf("CUDA kernel launch with %dx%d blocks of %dx%d threads\n", blocksPerGridy, blocksPerGridx, threadsPerBlock, threadsPerBlock);
	histogram_equalizationGPU<<<grid, threads>>>(d_img_out, d_img_in, d_hist_out, h_img_in.h * h_img_in.w, 256, d_lut, min, cdf, d);
	cudaCheckError();
